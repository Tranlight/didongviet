...
class RequestController extends Controller {
	...
	public function product_page($current_page) {
		...
		foreach ($ls_products as $product) {
			extract($product);
			$html .= "<tr data-product=$id><td><input type=checkbox name=cbox></td><td>$id</td><td><img height=30px src=". asset(explode('|', $url_images)[0])."></td><td><a href=\"/product$alias\">$name</a></td><td><button title=\"Edit\" class=\"btn btn-warning\"><i class=\"fal fa-edit\"></i></button><button title=\"Delete\" class=\"btn btn-danger\"><i class=\"fal fa-trash-alt\"></i></button></td></tr>";
		}
		$data['html'] = $html;
		header("Content-type: application/json");
	    echo json_encode($data);
